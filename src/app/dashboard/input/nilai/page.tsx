...
const InputNilai: React.FC = () => {
  ...
  const filteredMK = MK.filter((mk) => mk.kode.toLowerCase().includes(searchMK.toLowerCase()))

  if (PCPMK.length !== 0) {
    filteredPCPMK = PCPMK.filter((pcpmk) => pcpmk.kode.toLowerCase().includes(searchPCPMK.toLowerCase()))
  }
  ...
  const getMK = async (accountData: Account, tahunId: string) => {
    try {
      ...
      const userMKIds: string[] = accountData.kelas
        .filter((kelas) => kelas.tahunAjaran.id === Number.parseInt(tahunId))
        .map((kelas) => kelas.MKId)
      const response = await axiosConfig.get(`api/mk?prodi=${accountData.prodiId}&limit=99999`)
      if (response.data.status !== 400) {
        const filteredMK = response.data.data.filter((mk: MK) => userMKIds.includes(mk.kode))
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    const data = values.nilai.map((item) => ({
      ...
      nilai: item.nilai.map((nilai) => Number.parseFloat(nilai)),
      ...
      {kunciSistem?.nilai ? (
        <Card className="w-[1000px]">
          ...
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <h2 className="text-2xl font-semibold text-center mb-2">Belum Waktunya Input Nilai</h2>
            <p className="text-gray-500 text-center max-w-md">
              Sistem input nilai belum dibuka. Silahkan coba lagi nanti atau hubungi administrator untuk informasi lebih
              lanjut.
            </p>
          </CardContent>
        </Card>
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      >
                        <FormControl>
                          <SelectTrigger>
                            {field.value ? <SelectValue placeholder="Pilih MK" /> : "Pilih MK"}
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent>
                          <Input
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <Select
                        onValueChange={(value) => {
                          field.onChange(value)
                          setSelectedKelas(selectedMK?.kelas.find((kelas) => kelas.id === Number.parseInt(value)))
                          form.resetField("nilai")
                        }}
                      ...
                      >
                        <FormControl>
                          <SelectTrigger>
                            {field.value ? <SelectValue placeholder="Pilih Kelas" /> : "Pilih Kelas"}
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent>
                          {selectedMK?.kelas
                            .filter((kelas) => accountData?.kelas.some((accKelas) => accKelas.id === kelas.id))
                            .filter((kelas) => kelas.tahunAjaran.id === Number.parseInt(selectedTahun))
                            .map((kelas) => (
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <Select
                        onValueChange={(value) => {
                          field.onChange(value)
                          const selectedPCPMK = PCPMK.find((pcpmk) => pcpmk.kode === value)
                      ...
                      >
                        <FormControl>
                          <SelectTrigger>
                            {field.value ? <SelectValue placeholder="Pilih PCPMK" /> : "Pilih PCPMK"}
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent>
                          <Input
                          ...
                          <tr key={mIndex} className="border-t">
                            ...
                            {selectedPCPMK.kriteria.map((kriteria, kIndex) => (
                              <td key={kIndex} className="py-2 px-4 border">
                                <FormField
                                  control={form.control}
                                  name={`nilai.${mIndex}.nilai.${kIndex}` as const}
                                  render={({ field }) => (
                                    <FormItem className="m-0">
                                      <Input
                                        ...
                                        value={field.value ?? ""}
                                        onChange={(e) => {
                                          const updatedNilaiArray = [...(form.getValues(`nilai.${mIndex}.nilai`) || [])]
                                          updatedNilaiArray[kIndex] = e.target.value
                                          form.setValue(`nilai.${mIndex}.nilai`, updatedNilaiArray)
                                          form.setValue(`nilai.${mIndex}.mahasiswa`, mahasiswa.nim)
                                        }}
                                      />
                                      <FormMessage />
                                    </FormItem>
